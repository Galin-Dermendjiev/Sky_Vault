...
export async function uploadFile({file, ownerId, accountId, path}: UploadFileProps) {
    ...
    try {
        ...
        const bucketFile = await storage.createFile({bucketId: appwriteConfig.bucketId, fileId: ID.unique(), file: inputFile})
        ...
        const newFile = await tables.createRow({
...
export async function getFiles(){
    ...
    try{
        ...
        const files = await tables.listRows({databaseId: appwriteConfig.databaseId, tableId: appwriteConfig.filesTableId, queries})
